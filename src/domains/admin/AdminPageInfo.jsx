...
import { faEdit, faChevronDown, faTimes } from '@fortawesome/free-solid-svg-icons';
    ...
    return (
        <div className="h-full flex flex-col bg-white font-sans p-3 md:p-8">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-3 md:gap-4 mb-4 md:mb-6 flex-shrink-0">
                ...
                <button
                    className="md:hidden self-end px-3 py-1.5 bg-[#3530B8] text-white rounded-lg text-xs font-bold"
            ...
            {/* 탭 영역 */}
            <div className="flex gap-8 border-b border-gray-100 mb-6 overflow-x-auto no-scrollbar flex-shrink-0">
                {['전체', ...categories].map((category, idx) => (
                    <button
                        ...
                        className={`pb-3 text-sm font-bold transition-all relative flex-shrink-0 whitespace-nowrap ${activeTab === category
                            ? 'text-[#3530B8]'
            ...
            <div className="flex-1 flex gap-6 min-h-0 overflow-hidden">
                    ...
                    <div className="flex-1 overflow-auto custom-scrollbar">
                        <table className="w-full min-w-[600px]">
                            <thead className="bg-white border-b border-gray-50 sticky top-0 z-10">
                                <tr>
                                    <th className="py-4 px-6 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-1/6 whitespace-nowrap">카테고리</th>
                                    <th className="py-4 px-6 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-1/4 whitespace-nowrap">페이지명</th>
                                    <th className="py-4 px-6 text-left text-xs font-bold text-gray-500 uppercase tracking-wider flex-1 min-w-[200px]">안내 문구</th>
                                    <th className="py-4 px-6 text-center text-xs font-bold text-gray-500 uppercase tracking-wider w-24 whitespace-nowrap">관리</th>
                                </tr>
                            </thead>
                                ...
                                {filteredPages.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((page) => (
                                    editRowId === page.page_seq ? (
                                        <tr key={page.page_seq} className="bg-slate-50 transition-colors">
                                            <td className="py-4 px-6 relative">
                                                <div
                                                    ...
                                                    className="w-full px-4 py-2 bg-gray-50 border border-gray-100 rounded-lg text-xs font-bold text-gray-700 flex items-center justify-between gap-2 whitespace-nowrap cursor-pointer hover:border-[#3530B8] transition-all"
                                                >
                                                        ...
                                                        <div className="absolute top-full left-6 right-6 mt-1 z-[100] bg-white border border-gray-100 rounded-xl shadow-lg max-h-48 overflow-y-auto custom-scrollbar animate-in fade-in zoom-in-95">
                                                            {categories.filter(c => c !== '전체').map(c => (
                                                                <div
                                                                    ...
                                                                    className={`p-3 hover:bg-[#F0F4FF] cursor-pointer text-xs font-bold text-gray-700 border-b border-gray-50 last:border-0 whitespace-nowrap ${editRowData.page_category === c ? 'bg-[#F0F4FF]' : ''}`}
                                                                >
                                                                    {c}
                                                                </div>
                                                            ))}
                                            ...
                                            <td className="py-4 px-6 text-sm font-semibold text-slate-700 whitespace-nowrap">{page.page_name}</td>
                                        ...
                                        <tr key={page.page_seq} className="hover:bg-slate-50/50 transition-colors">
                                            <td className="py-4 px-6 text-sm font-semibold text-slate-700 whitespace-nowrap">{page.page_category}</td>
                                            <td className="py-4 px-6 text-sm font-semibold text-slate-700 whitespace-nowrap">{page.page_name}</td>
                ...
                {/* 우측 카테고리 관리 영역 */}
                <div className={`${isMobileCategoryModalOpen ? 'fixed inset-0 z-50 p-4 bg-black/50 backdrop-blur-sm flex items-center justify-center' : 'hidden'} md:relative md:inset-auto md:p-0 md:bg-transparent md:backdrop-blur-none md:flex md:w-80 md:flex-col md:z-auto`}>
                    <div className={`bg-white flex flex-col shadow-sm min-h-0 ${isMobileCategoryModalOpen ? 'w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl max-h-[80vh]' : 'w-full rounded-[2rem] border border-[#F0F4FF] md:h-[73%] flex-shrink-0'}`}>
                        {/* Header for mobile */}
                        <div className="p-4 border-b border-gray-50 flex justify-between items-center md:hidden">
                            <h2 className="text-sm font-bold text-slate-800">카테고리 관리</h2>
                            <button onClick={() => setIsMobileCategoryModalOpen(false)}>
                                <FontAwesomeIcon icon={faTimes} className="text-gray-400" />
                            </button>
                        </div>
                        {/* Header for desktop */}
                        <div className="p-6 border-b border-gray-50 flex-shrink-0 hidden md:flex">
                            <h2 className="text-lg font-bold text-gray-900">카테고리 관리</h2>
                        </div>

                        <div className="flex-1 overflow-y-auto px-6 pb-6 pt-4 md:pt-0 custom-scrollbar space-y-2">
